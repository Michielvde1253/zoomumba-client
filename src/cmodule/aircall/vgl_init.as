package cmodule.aircall
{
   ...
   public function vgl_init(param1:int, param2:int, param3:int) : int
   {
      ...
      stage = cmodule.aircall.gsprite.stage;
      trace("vgl_init: " + width + " / " + height + " : " + pixels);
      if(cmodule.aircall.vglKeyFirst)
      {
         ...
         {
            var _loc2_:int = cmodule.aircall.vglKeyMode == 2 ? int(vk2scan[param1.keyCode & 0x7F]) : int(param1.charCode);
            cmodule.aircall.vglKeys.push(_loc2_);
         });
         stage.addEventListener(KeyboardEvent.KEY_UP,function(param1:KeyboardEvent):*
         {
            var _loc2_:int = cmodule.aircall.vglKeyMode == 2 ? int(vk2scan[param1.keyCode & 0x7F]) : int(param1.charCode);
            if(cmodule.aircall.vglKeyMode == 2)
            {
               cmodule.aircall.vglKeys.push(_loc2_ | 0x80);
            }
         });
         cmodule.aircall.vglKeys.push(69);
         stage.focus = stage;
         cmodule.aircall.vglKeyFirst = false;
      }
      cmodule.aircall.gvglpixels = pixels;
      cmodule.aircall.gvglbmd = new BitmapData(Math.abs(width),Math.abs(height),false);
      if(!cmodule.aircall.gvglbm)
      {
         cmodule.aircall.gvglbm = new Bitmap();
         cmodule.aircall.gsprite.addChild(cmodule.aircall.gvglbm);
      }
      cmodule.aircall.gvglbm.bitmapData = cmodule.aircall.gvglbmd;
      cmodule.aircall.gvglbm.scaleX = cmodule.aircall.gsprite.stage.stageWidth / width;
      cmodule.aircall.gvglbm.scaleY = cmodule.aircall.gsprite.stage.stageHeight / height;
      trace("vgl_init done");
      return 0;
   }
}
