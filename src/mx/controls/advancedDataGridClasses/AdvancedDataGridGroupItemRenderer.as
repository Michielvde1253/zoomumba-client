package mx.controls.advancedDataGridClasses
{
   ...
   public class AdvancedDataGridGroupItemRenderer extends UIComponent implements IDataRenderer, IDropInListItemRenderer, IListItemRenderer, IFontContextComponent
   {
      ...
      override protected function commitProperties() : void
      {
         ...
         if(Boolean(hasFontContextChanged()) && this.label != null)
         {
            _loc3_ = int(getChildIndex(DisplayObject(this.label)));
            this.removeLabel();
            this.createLabel(_loc3_);
         }
         if(this.icon)
         {
            _loc1_ = Number(this.icon.x);
            removeChild(DisplayObject(this.icon));
            this.icon = null;
         }
         if(this.disclosureIcon)
         {
            _loc2_ = Number(this.disclosureIcon.x);
         ...
         if(this._data != null)
         {
            ...
            this.label.wordWrap = this.listOwner.mx_internal::columnWordWrap(_loc4_);
      ...
      override protected function updateDisplayList(param1:Number, param2:Number) : void
      {
         ...
         if(Boolean(this.data) && Boolean(parent))
         {
            if(!enabled)
            {
               _loc5_ = Number(getStyle("disabledColor"));
            }
            else if(this.listOwner.isItemHighlighted(this.listData.uid))
            {
               _loc5_ = Number(getStyle("textRollOverColor"));
            }
            else if(this.listOwner.isItemSelected(this.listData.uid))
            {
               _loc5_ = Number(getStyle("textSelectedColor"));
            }
            else
            {
               _loc5_ = Number(getStyle("color"));
            }
      ...
      private function disclosureMouseDownHandler(param1:Event) : void
      {
         ...
         this.listOwner.mx_internal::dispatchAdvancedDataGridEvent(AdvancedDataGridEvent.ITEM_OPENING,this._listData.item,this,param1,!_loc2_,true,true);
      }
