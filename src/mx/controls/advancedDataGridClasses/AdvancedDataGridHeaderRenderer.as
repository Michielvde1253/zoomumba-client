   ...
   public class AdvancedDataGridHeaderRenderer extends UIComponent implements IDataRenderer, IDropInListItemRenderer, IListItemRenderer
   {
      ...
      override protected function commitProperties() : void
      {
         ...
         if(this._data != null)
         {
            ...
            {
               this.label.wordWrap = this.grid.mx_internal::columnHeaderWordWrap(this._data as AdvancedDataGridColumn);
            }
            else
            {
            ...
            if(this._data is AdvancedDataGridColumn)
            {
               ...
               if(_loc3_)
               {
                  if(this.label.textWidth > this.label.width || _loc2_.dataTipFunction || _loc2_.dataTipField || this.grid.dataTipFunction || Boolean(this.grid.dataTipField))
                  {
      ...
      override protected function measure() : void
      {
         ...
         var _loc5_:Number = !!this.sortItemRendererInstance ? Number(this.sortItemRendererInstance.getExplicitOrMeasuredWidth()) : 0;
         var _loc6_:Number = !!this.sortItemRendererInstance ? Number(this.sortItemRendererInstance.getExplicitOrMeasuredHeight()) : 0;
         if(this.grid.sortExpertMode && this.getFieldSortInfo() == null)
         {
            _loc5_ = 0;
            _loc6_ = 0;
         }
         var _loc7_:Number = Number(getStyle("horizontalGap"));
         if(_loc5_ == 0)
         {
            _loc7_ = 0;
         }
         ...
         if(!isNaN(explicitWidth))
         {
            ...
            _loc9_ = this.label.textHeight + UITextField.mx_internal::TEXT_HEIGHT_PADDING;
         }
         else
         {
            _loc12_ = measureText(this.label.text);
            _loc8_ = _loc12_.width + UITextField.mx_internal::TEXT_WIDTH_PADDING;
            _loc9_ = _loc12_.height + UITextField.mx_internal::TEXT_HEIGHT_PADDING;
            _loc10_ = _loc8_ + _loc7_ + (!!this.partsSeparatorSkin ? this.partsSeparatorSkin.width : 0) + _loc5_;
         }
         _loc11_ = Math.max(_loc9_,_loc6_);
         _loc11_ = Math.max(_loc11_,!!this.partsSeparatorSkin ? this.partsSeparatorSkin.height : 0);
      ...
      override protected function updateDisplayList(param1:Number, param2:Number) : void
      {
         ...
         var _loc7_:Number = !!this.sortItemRendererInstance ? Number(this.sortItemRendererInstance.getExplicitOrMeasuredWidth()) : 0;
         var _loc8_:Number = !!this.sortItemRendererInstance ? Number(this.sortItemRendererInstance.getExplicitOrMeasuredHeight()) : 0;
         ...
         var _loc12_:Number = _loc11_.width + UITextField.mx_internal::TEXT_WIDTH_PADDING;
         ...
         var _loc15_:Number = this.label.textHeight + UITextField.mx_internal::TEXT_HEIGHT_PADDING;
         ...
         if(this.sortItemRendererInstance && !this.grid.sortExpertMode && !(this._data is AdvancedDataGridColumnGroup))
         {
            ...
            {
               graphics.lineStyle(1,getStyle("separatorColor") !== undefined ? uint(getStyle("separatorColor")) : 13421772);
         ...
         if(Boolean(this.data) && Boolean(parent))
         {
            if(!enabled)
            {
               _loc21_ = Number(getStyle("disabledColor"));
            }
            else if(this.grid.isItemHighlighted(this.listData.uid))
            {
               _loc21_ = Number(getStyle("textRollOverColor"));
            }
            else if(this.grid.isItemSelected(this.listData.uid))
            {
               _loc21_ = Number(getStyle("textSelectedColor"));
            }
            else
            {
               _loc21_ = Number(getStyle("color"));
            }
