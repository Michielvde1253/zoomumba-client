package cmodule.aircall
{
   ...
   public class CRunner implements Debuggee
   {
      ...
      {
         super();
         if(cmodule.aircall.grunner)
         {
            log(1,"More than one CRunner!");
         }
         cmodule.aircall.grunner = this;
      ...
      public function startInit() : void
      {
         ...
         log(2,"Static init...");
         modStaticInit();
         args = cmodule.aircall.gstate.system.getargv();
         env = cmodule.aircall.gstate.system.getenv();
         argv = this.createArgv(args);
         envp = this.createEnv(env);
         startArgs = [args.length].concat(argv,envp);
         ap = this.rawAllocIntArray(startArgs);
         cmodule.aircall.gstate.ds.length = cmodule.aircall.gstate.ds.length + 4095 & ~0x0FFF;
         cmodule.aircall.gstate.push(ap);
         cmodule.aircall.gstate.push(0);
         log(2,"Starting work...");
         ...
         {
            cmodule.aircall.gstate.system.exit(e.rv);
            return;
         }
            ...
            do
            {
               ...
               while(checkInterval > 0)
               {
                  try
                  {
                     while(checkInterval-- > 0)
                     {
                        cmodule.aircall.gstate.gworker.work();
                     }
                  }
                  catch(e:AlchemyDispatch)
                  {
      ...
      public function startSystemBridge(param1:String, param2:int) : void
      {
         ...
         cmodule.aircall.gstate.system = new CSystemBridge(param1,param2);
         cmodule.aircall.gstate.system.setup(this.startInit);
      }
      ...
      public function rawAllocString(param1:String) : int
      {
         var _loc2_:int = 0;
         var _loc3_:int = 0;
         _loc2_ = int(cmodule.aircall.gstate.ds.length);
         cmodule.aircall.gstate.ds.length += param1.length + 1;
         cmodule.aircall.gstate.ds.position = _loc2_;
         _loc3_ = 0;
         while(_loc3_ < param1.length)
         {
            cmodule.aircall.gstate.ds.writeByte(param1.charCodeAt(_loc3_));
            _loc3_++;
         }
         cmodule.aircall.gstate.ds.writeByte(0);
         return _loc2_;
      }
      ...
      public function rawAllocIntArray(param1:Array) : int
      {
         ...
         var _loc3_:int = 0;
         _loc2_ = int(cmodule.aircall.gstate.ds.length);
         cmodule.aircall.gstate.ds.length += (param1.length + 1) * 4;
         cmodule.aircall.gstate.ds.position = _loc2_;
         _loc3_ = 0;
         while(_loc3_ < param1.length)
         {
            cmodule.aircall.gstate.ds.writeInt(param1[_loc3_]);
            _loc3_++;
         }
         return _loc2_;
      }
      ...
      {
         log(3,"local system");
         cmodule.aircall.gstate.system = new CSystemLocal(param1);
         cmodule.aircall.gstate.system.setup(this.startInit);
      }
