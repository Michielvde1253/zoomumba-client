   ...
   internal class CProcTypemap extends CTypemap
   {
      ...
      override public function createC(param1:*, param2:int = 0) : Array
      {
         ...
         id = regFunc(function():void
         {
            ...
            var ts:* = undefined;
            args = [];
            cmodule.aircall.mstate.pop();
            sp = cmodule.aircall.mstate.esp;
            n = 0;
            while(n < argTypemaps.length)
            {
               ...
               aa = [];
               ts = tm.typeSize;
               cmodule.aircall.mstate.ds.position = sp;
               sp += ts;
               while(ts)
               {
                  aa.push(cmodule.aircall.mstate.ds.readInt());
                  ts -= 4;
               }
            ...
            try
            {
               retTypemap.toReturnRegs(cmodule.aircall.mstate,v.apply(null,args));
            }
            catch(e:*)
            {
               cmodule.aircall.mstate.eax = 0;
               cmodule.aircall.mstate.edx = 0;
               cmodule.aircall.mstate.st0 = 0;
      ...
      override public function fromC(param1:Array) : *
      {
         ...
         return function(... rest):*
         {
            ...
            var cleanup:Function = function():void
            {
               ...
               cmodule.aircall.mstate.esp = sp;
               cmodule.aircall.mstate.gworker = oldWorker;
            };
            sp = cmodule.aircall.mstate.esp;
            cargs = [];
            oldWorker = cmodule.aircall.mstate.gworker;
            if(async)
            {
               asyncHandler = args.shift();
               cmodule.aircall.mstate.gworker = new NotifyMachine(function():Boolean
               {
                  var result:* = undefined;
                  result = retTypemap.fromReturnRegs(cmodule.aircall.mstate);
                  cleanup();
                  try
                  {
                     asyncHandler(result);
            ...
            if(!asyncHandler)
            {
               try
               {
                  try
                  {
                     cmodule.aircall.mstate.funcs[int(v[0])]();
                  }
                  ...
                  while(cmodule.aircall.mstate.gworker !== oldWorker)
                  {
                     try
                     {
                        while(cmodule.aircall.mstate.gworker !== oldWorker)
                        {
                           cmodule.aircall.mstate.gworker.work();
                        }
                     }
                     catch(e:AlchemyYield)
                     {
                  ...
                  return retTypemap.fromReturnRegs(cmodule.aircall.mstate);
               }
               finally
               {
                  cleanup();
      ...
      private function push(param1:*) : void
      {
         ...
         if(param1 is Array)
         {
            ...
            while(_loc2_ >= 0)
            {
               cmodule.aircall.mstate.push(param1[_loc2_]);
               _loc2_--;
            }
         }
         else
         {
            cmodule.aircall.mstate.push(param1);
         }
      }
   }
}
