package com.bigpoint.zoomumba.view.cards.custom
{
   ...
   public class InventoryBreedingLabCardMediator extends BaseCardMediator
   {
      public static const NAME:String = "InventoryBreedingLabCardMediator_";
      
      public var cardData:BreedingLabShopData;
      
      public var datas:Array;
      
      public function InventoryBreedingLabCardMediator(param1:Array)
      {
         this.cardData = param1[0];
         this.datas = param1;
         _inventoryMode = true;
         super(getNameFromPrefix(NAME,Categories.BREEDING_LAB,this.cardData.itemId),param1[0],new ItemCard());
      }
      ...
      override protected function handleSellItem(param1:MouseEvent) : void
      {
         ...
         {
            _loc4_.push(BreedingLabShopData(this.datas[_loc5_]).uniqueId);
            _loc5_++;
         }
      ...
      override public function handleNotification(param1:INotification) : void
      {
         ...
         switch(param1.getName())
         {
            case CARDS.CHANGE_INVENTORY_AMOUNT:
               ...
               _loc3_ = int(this.datas.length);
               if(this.datas.length == 0)
               {
                  return;
               }
               if(_loc2_[0] != this.cardData.categoryId || _loc2_[1][0] != BreedingLabShopData(this.datas[0]).uniqueId)
               {
                  return;
               }
               _loc4_ = int(_loc2_[1].length);
               this.datas.splice(0,_loc4_);
               if(card)
               {
                  card.topGroup.counter.value = _loc3_ - _loc4_;
               }
               if(_loc3_ - _loc4_ == 0)
               {
      ...
      {
         var _loc1_:String = TextIdHelper.getCardName(Categories.BREEDING_LAB,this.cardData.itemId);
      ...
      {
         this.cardData.localisedInfoId = TextIdHelper.getCardInfo(Categories.BREEDING_LAB,this.cardData.itemId);
      }
